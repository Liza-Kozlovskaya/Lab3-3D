...
namespace Lab3
{
    public class Normal
    {
        ...
        public static PolygonNormals CalculateNormals(Point3D[] cubePoints)
        {
            ...
            //поиск для каждой стороны нормали
        ...
        {
            var xR = (2 * normal.X * (normal.X * light.X + normal.Y * light.Y + normal.Z * light.Z) / 
                Math.Sqrt(Math.Pow(normal.X, 2) + Math.Pow(normal.Y, 2) + Math.Pow(normal.X, 2))) - light.X;

            var yR = (2 * normal.Y * (normal.X * light.X + normal.Y * light.Y + normal.Z * light.Z) / 
                Math.Sqrt(Math.Pow(normal.X, 2) + Math.Pow(normal.Y, 2) + Math.Pow(normal.X, 2))) - light.Y;

            var zR = (2 * normal.Z * (normal.X * light.X + normal.Y * light.Y + normal.Z * light.Z) / 
                Math.Sqrt(Math.Pow(normal.X, 2) + Math.Pow(normal.Y, 2) + Math.Pow(normal.X, 2))) - light.Z;
        ...
        // Ia - интенсивность рассеяного света
        // Ks - зеркальное отражение
        // Kd - диффузное отражение
        // Ka - константа
        // cosA - угол между наблюдателем и отраж. лучем
        // cosQ - угол между источником света и нормалью
